...
`SequentialFigure7CommitmentEdgeTargetAvoidance.lean` closes the conditional
one-edge avoidance step. Given a `FutureNewCandidateAt` and an explicit law
that the exact child ledger event does not touch that candidate's tensor
...
`SequentialFigure7OlderEventTouchSeparation.lean` restates exactly that
geometry in history-friendly touch language.
`ReservationEvent.TouchSeparatedFrom` says that one authentic event touched no
occurrence in the candidate region. Region disjointness always yields this
predicate; under `StructurallyWellFormed`, reservation-event touch completeness
proves the converse. `OlderEventTouchSeparated` preserves the old invariant's
exact ledger membership, future-candidate quantification, and strict current-
representative antecedent, and Lean proves the two history predicates
...
complete `NewSourceRegionInput`, or contains an exact old marked occurrence
owner; equivalently, `NewEnabled` holds or that owner exists. An explicit
pointwise no-owner premise therefore yields `NewEnabled`. The successful
branch already includes the exact run, raw and endpoint readiness, endpoint
queue absence, fresh waiting capacity, and history-level
`FutureWaitingUndefined`. This remains a conditional reduction: the module
...
`SequentialFigure7OlderEventFutureWorkTouchSeparation.lean` isolates the exact
queued-head residue left by tensor-conclusion touch decomposition.
`OlderEventFutureWorkTouchSeparated` says that a strictly older authentic
ledger event does not touch any future-New candidate's exact queued head.
Together with `OlderEventTouchSeparated`, the structural mate-or-head split
therefore excludes a strictly older event from the candidate tensor conclusion
...
invariants, unconditional stored-left equal-boundary avoidance, enabledness,
...
`SequentialFigure7EqualBoundaryCommitmentTargetAvoidance.lean` classifies that
final edge without assuming it away. In the stored-right tensor orientation,
the same-representative touch decomposition contradicts orientation and yields
the canonical path avoiding the active tensor conclusion. In general Lean gives
an inclusive dichotomy: either that avoiding path exists, or an authentic child
event has the active raw age, stored-left orientation, and an exact adjacent
`tensor conclusion :: queued head` trace fragment. The obstruction branch
witnesses failure of the generic child-untouched callback; it does not assert
that every avoiding path is absent, and both branches may hold. This closes no
mate-region or raw-mark invariant, queue origin, progress, totality,
completeness, fallback-removal, scheduling, or complexity obligation.
`SequentialFigure7CommitmentBlockerAdvance.lean` combines the globally
available queued-head separation law with the strict `sigma` split and this
...
Retained candidates transport through the stable theorem. The newly appended
reservation event cannot be strictly older than any output candidate because
its fresh boundary is the maximal output raw age. The remaining old-event /
...
unconditional `wait` preservation.
`SequentialFigure7OlderEventFutureWorkTouchWaitPreservation.lean` gives the
parallel queued-head result. For an already-successful typed `WaitStep`,
retained candidates transport through the destination and prepared prefix.
The candidate-indexed `WaitCreatedHeadTouchSeparated` premise states exactly
the remaining old-event touch obligation when the inserted conclusion is an
actual future-New candidate; Wait itself appends no reservation event.
Relative to the supplied prior canonical history and queued-head invariant,
that premise is the exact transition-local residual. The module does not
...
antecedent for a candidate created there. Retained candidates transport through
Prepared preservation. The only residual case is an input-retained raw mark
against an actual `WaitCreatedCandidate`, stated by
`WaitRetainedRawMarksSeparated`. This transition-local condition is distinct
from the history-side `WaitCreatedRegionSeparated`; the theorem assumes neither
declarative correctness nor history/reachability. It is still conditional and
does not establish the residual premise, unconditional Wait, or progress.
...
gives the parallel queued-head result. Survivor candidates transport the prior
invariant directly; active-bucket work moved to the previous boundary is
recovered as a prepared active-boundary candidate. A strict output comparison
excludes an older event from the retired active representative class, so its
representative is unchanged by the union. The sole created case uses the
candidate-indexed `UnifyPayloadCreatedHeadTouchSeparated` premise for the
inserted tensor conclusion. UnifyPayload appends no reservation event.
Relative to the supplied prior invariant this premise is the exact
transition-local residual. The module does not derive it from scheduler
...
New-created candidates (59,706 reached endpoints and 162,540 partner
...
- an event-driven worklist prototype that precomputes premise consumers,
  ...
  totality, unconditional reachability, global availability of the mate-region
  separation invariant, unconditional stored-left equal-boundary avoidance,
...
lake exe proofnet_ir_equal_boundary_commitment_target_avoidance_tests
lake exe proofnet_ir_commitment_blocker_advance_tests
...
ProofNetIR/SequentialFigure7EqualBoundaryCommitmentTargetAvoidance.lean
  stored-right final-edge avoidance and inclusive stored-left touch-witness split
ProofNetIR/SequentialFigure7CommitmentBlockerAdvance.lean
  current-representative path-or-higher-mate-touch-or-equal-callback reduction
...
ProofNetIREqualBoundaryCommitmentTargetAvoidanceTests.lean
  equal-boundary stored-right avoidance and inclusive dichotomy consumer
ProofNetIRCommitmentBlockerAdvanceTests.lean
  current-representative blocker-advance three-way reduction consumer
