...
- extended `--invariant-probe` with C6 (mate in the active bucket), C7 (bucket
  ...
  reachable state: when the active bucket's head is a par premise whose
  mate is unmarked or marked below the active age, the rest of the bucket
  holds a non-conclusion. C6, C7, and C11 fail; C8 shows at least two raw
  vertices at every nop/wait pre-state; C12 holds at all 1,217,664 default
  and 1,071,360 wait-focus reachable states and is exactly the nop/wait
  obligation of H-tail as a state predicate. Nothing is proved and no
  ledger item is closed;
...
- added the strict-singleton Figure-7 `UnifyOne` slice for exactly
  ...
  one atomic prepare → tensor construction/union → waiting-par activation →
  scheduler-drain transition; its exact state equation increments the
  connective counter by two. Independent Boolean-free
  `WaitingParActivationRule` and `UnifyOneRule` relations have typed/executable
  soundness, completeness, iff, and output-uniqueness theorems under their
  documented structural, invariant, and list-shape hypotheses. Successful
  typed and executable steps preserve both `ReservationInvariant` and the
  complete occurrence-exact `SchedulerInvariant`. Empty and length-at-least-two
...
- added the deliberately bounded Figure-7 `UnifyEmpty` slice. The executable
  accepts only an initialized empty previous waiting cell, retains the exact
  submitted tensor slot and stored premise orientation, and checks the literal
  raw-age guard `j ≤ μ(mate) < i`. `RealizesSigma` plus the new adjacent-boundary
  interval theorem identifies the production representatives as exactly `j`
  and `i`; the core therefore merges the two exact components while the stack
  pops the active level, drains `W(j) = []`, and installs
  `conclusion :: (previousReady ++ activeReady)`. An independent direct
  `UnifyEmptyRule` states the mutation equations without invoking the
  ...
  `parent[i] := j`. With a full input `SchedulerInvariant`, successful typed and
  executable bounded steps additionally preserve the complete occurrence-exact
  invariant: exact tensor-derived component ownership, the surviving/retired
  forest update, live frontiers, queue/waiting/pending facts, and the fired
  counter. Strict-singleton waiting-par activation is now covered separately
  by `UnifyOne`; arbitrary payloads of length at least two, later-state
  `forward?` applicability/totality, dispatcher/history integration, full-rule
  reachability, correct-state progress, pure-worklist completeness,
  recursive-fallback removal, and whole-program linearity remain open;
...
- added `SequentialFigure7Rules.lean`, with a proof-carrying generic
  ...
  synchronized common pop/raw-mark prefix. Executable `concl?`, `nop?`, and
  local `wait?`
  ...
  `ConclRule`, `NopRule`, and `WaitRule` relations, executable soundness,
  completeness
  under `StructurallyWellFormed` plus `ReservationInvariant`, and output
  uniqueness (for `wait`, under structural validity and the supplied
  invariant). `wait?` compares the
  mate's raw mark to the selected raw age, uses
  `sigmaBoundary? stack.sigma mateRawAge` as its only destination, fails
  closed on out-of-bounds or undefined cells, and prepends the par conclusion
  by bucket-local O(1) cons without a `queuedVertices` scan. Tail-based
  ready/`sigma` operations, boundary lookup, and the overall transition carry
  no O(1) or linearity claim. `WaitRule` states this as a
  Boolean-free proposition with an exact boundary equation and the guard in
  `before.core.marks`. The dependent
...
- proved the later `RealizesSigma` bridge. Appending a reservation uses
  ...
  ready/waiting payload ownership through the arbitrary `unify` fold,
  full-history integration of local
  `concl`/`nop`/`wait`/`forward`/`UnifyEmpty`/`UnifyOne`, later totality,
...
- added `ProofNetIR/SequentialSchedulerState.lean` as the first independent
  ...
  remained open. The newer successful local `forward` and bounded
  `UnifyEmpty` checkpoints are described above;
- added `ProofNetIR/SequentialUnification.lean` as the first bounded
  ...
  The newer successful local `forward` and bounded `UnifyEmpty` checkpoints are
  described above;
...
- added a private proof-relevant structural seam replay for the unified exact
  ...
  once, replays its source-fixed reverse shells and nonempty omitted arc, folds
  every generator-exact backward-search and nested-terminal frame, and reaches
  the package's initial tagged family with a nonempty accumulated candidate
  gap. The replay does not use the legacy `CyclicIntervalDescent`, a bare
  `CyclicIntervalCut`, or an independently reselected frame. The candidate gap
  is not proved to be a contiguous scheduler slice or an endpoint adjacency,
  and the generic replay alone does not retain each frame's scheduler-origin
  and reverse-shell relation. A semantic invariant must therefore consume the
  exact package/trace alongside the replay, or strengthen the replay with typed
  origin evidence, before closing-par exclusion, progress, or completeness can
  be claimed.
- assembled the surviving closing case into one data-indexed exact package.
  ...
  but does not prove uniqueness, canonicality, closing-par
...
- strengthened the sole surviving tagged closing-par base without claiming its
  ...
  `CyclicIntervalDescent`. Every recursive backward step now stores a
  `SchedulerTaggedBackwardParCut` generated by the same coordinate-exact
  positioned obstruction: the omitted-right and retained-left tags, both
  cyclic append decompositions, backward orientation, retained suffix, and
  strict cut are one semantic relation. The cut projection reconstructs its
  strict interval from those exact equations and nonempty discarded prefix;
  it does not retain a second independently destructible cut decomposition.
  ...
  step wrapper. The newer structural replay above now consumes that hidden
  `StepAt` frame without selecting a replacement. A scheduler-specific semantic
  invariant over the replay and exact trace (or a separate residual-derivation
  progress proof) is still required before the closing base, correct-state
  progress, pure-worklist completeness, and fallback removal can be claimed;
