...
Every later kernel-checked consequence of that interpretation (the operational
`new`, the local rules, the canonical dispatcher, histories, region and
commitment invariants, the tail-law reduction, and the termination bound) is
a code result, not a new literature reading; the coverage matrix above is
unchanged by them. The exact code-level mapping and nonclaims are maintained in
[guerrini-unification-audit.md](guerrini-unification-audit.md), the layer
structure in [architecture.md](architecture.md#sequential-figure-7-layer),
and the open items in the [goal ledger](goal-ledger.md).
