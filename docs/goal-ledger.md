...
| --- | --- | --- |
| D1 | **Open (retargeted 2026-09-20):** completeness of a checker-free fast path, now the sequential Figures 7–8 executable: `Certificate.sequentialFastCheck = Certificate.check`. | `Certificate.sequentialFastCheck` and `sequentialReconstruct?` exist in `ProofNetIR/Figure7/Sequential.lean` with `sequentialFastCheck_sound`; `runDispatcher_spec` gives the run's endpoint from D3 and D4 (reachable, fully marked, stopped). Initialization totality is `StructurallyWellFormed.initializeReservation?_isSome`. Proved in the same module: a fully marked reachable correct state has one live component owning exactly the input carrier; its frontier permutes the conclusions; final extraction succeeds with an exact occurrence exchange; the tree infers the input sequent and desequentializes to an accepted certificate. Remaining: the explicit fresh-numbering correspondence for par and tensor (axiom and exchange cases are proved), its full-carrier `ProofNetEquivalent` consequence, and verifier/reconstruction completeness. The two remaining propositions are stated as private target definitions at the end of that module; D1 remains open. The legacy flat-worklist form `Certificate.unificationWorklistFastCheck = Certificate.check` stays recorded below as D1-flat; it is still open, blocked by H-closing-par, and no longer on the critical path. Soundness of the flat path is `Certificate.unificationWorklistFastCheck_sound` (`ProofNetIR/Unification.lean`). |
