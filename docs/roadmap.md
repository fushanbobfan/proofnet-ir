...
- [x] Prove the reverse bridge from structural well-formedness, a connected
  ...
  - [ ] Retired 2026-09-21, unproved: prove
    ...
    - [x] Prove that canonical initialization plus the complete production
      ...
      pop-and-process scheduler coverage, reverse consumer-table and
      waiting-par provenance, exact queue/waiting flag completeness, and
      cumulative enqueue accounting; classify every unfired connective at
      quiescence as an idle premise, a registered distinct-thread par, or a
      same-thread tensor deadlock, and exclude the deadlock on correct inputs.
    - [x] Reduce the remaining waiting-par obstruction to a nonempty closed
      dependency cycle of the reference graph, normalize it through exact
      occurrence cancellation and cyclic reverse-shell normalization to a
      terminal forward retained-left par cusp, and assemble the surviving
      closing case into one data-indexed package with its endpoint zipper
      (`ProofNetIR/Unification.lean`, July 2026).
...
## v0.10 - Sequential Figures 7–8 scheduler

Exact target statements for the open items are in the
[goal ledger](goal-ledger.md); the layer structure is in
[architecture.md](architecture.md#sequential-figure-7-layer).

- [x] Add the reusable source-incidence index, the bounded globally tagged
  `NEXTAXIOM` search with exact trace and touched-set disjointness, the
  oriented source-left route theorem, and initial/local search totality.
- [x] Formalize the delayed raw-age stack (`σ` partition, three-way waiting
  cells, strict empty `init`), keep the printed fresh-cell `newEnqueue?` as
  an audit helper, and bridge the stack to the production core through typed
  initial and later reservations preserving `ReservationInvariant`.
- [x] Implement the local rules `concl`, `nop`, `new`, `wait`, `forward`,
  `UnifyEmpty`, `UnifyOne`, and arbitrary-payload `UnifyPayload` with typed
  step records, executable correspondence, and preservation of the complete
  occurrence-exact `SchedulerInvariant`.
- [x] Add the canonical fixed-precedence dispatcher, typed `DispatchStep`
  witnesses, proof-relevant executed histories, canonical tag histories with
  exact touch provenance and submitted-slot non-reuse, and the input-only
  enabledness layers for every rule.
- [x] Reify reservation history as an unforgeable ledger ordered by raw age,
  recover the event behind every touch, raw mark, and queued endpoint, and
  make the retained `σ` commitment spine exact with canonical reference paths.
- [x] Prove the region-separation invariant families
  (`OlderEventFutureWorkTouch*`, `OlderRawMarkedRegion*`,
  `OlderMarkedTensorPredecessor*`, `CrossRepresentative*`) by initialization
  and preservation through every creating rule, and classify every dynamic
  source-left blocker under an authentic history.
- [x] Isolate the active-top residual, prove
  `dispatch_or_activeTopDrained`, define `ActiveTopMarkedNonconclusionDebt`,
  and reduce Figure-7 progress to `CanonicalTagHistory.ActiveTopDebtTailLaw`;
  prove the Wait-output obstruction against the endpoint-local law.
- [x] Classify the `nop`/`wait` re-entry target under exact ready-tail failure
  through the commitment-interval, raw-return chronology, and waiting-mate
  families with current-state queue-status receipts (August 2026). These are
  conditional classifications and did not discharge the law.
- [x] Add the deterministic finite replay audit for the ready-head boundary
  (`proofnet_ir_new_progress_audit`) and the finite tail-law search with
  wait-focused and invariant-probe modes (`proofnet_ir_tail_law_search`).
- [x] Bound every executed dispatcher history by `certificate.formulas.size`
  successful calls (`dispatch_stops`, D4).
- [x] Add the convergence ledger and CI gate, and reduce the `nop`/`wait`
  branches of the tail law to the state predicate C12: proved after every
...
- [ ] Reach a linear whole-program bound (D6-linear): constant-time stack
  ...
  planned for v0.10.
- [x] Publish `v0.10.0`, verify release-candidate, automatic tag-push, and
