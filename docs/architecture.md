...
1. **Delayed state and bridge.** `SequentialSchedulerState` is the delayed
   stack specification: ready buckets, the `sigma` boundary list, waiting
   cells, and the immutable discovery-order `RawTokenAge`, which is never a
   union-find representative. `SequentialSchedulerBridge` couples that stack
   with the production `UnificationState` and the complete `NEXTAXIOM` tag
   array in `ReservationState`, and proves the initial and later
   `RealizesSigma` bridges. `SequentialUnification`, `SequentialRoute`,
   `SequentialConsumerIndex`, and `SequentialComponentProvenance` supply the
   source-incidence index, the bounded globally tagged `NEXTAXIOM` search with
   its exact trace, and component ownership.
2. **Local rules.** `SequentialFigure7Rules` gives the proof-carrying
   par/tensor consumer view and the common prepared prefix (synchronized pop
   and raw mark). `SequentialFigure7New` is the invariant-bound operational
   `new`; the literal printed fresh-cell update is kept only as an audit
   helper. `SequentialFigure7Unify`, `UnifyOne`, and `UnifyPayload` implement
   the tensor merge for empty, singleton, and arbitrary finite waiting
   payloads, rebuilding the active bucket as
   `conclusion :: (payload ++ previousReady ++ activeReady)`.
3. **State invariant.** `SequentialSchedulerInvariant` is the occurrence-exact
   `SchedulerInvariant`; `Figure7SuccessfulStep.schedulerInvariant` proves
   that every successful typed rule preserves it, with the arbitrary-payload
   case in `SequentialFigure7UnifyPayloadInvariant`.
4. **Dispatcher and enabledness.** `SequentialFigure7Dispatcher` runs
   `concl`, `nop`, `new`, `wait`, `forward`, `unifyPayload` in that fixed
   precedence; typed `DispatchStep` witnesses recover the exact rule taken,
   and `ReachableByImplementedDispatcher` names the states it reaches. The
   input-only layers `StableEnabled`, `PriorityEnabled`, `UnifyPayloadEnabled`,
   `NewInputNecessary`, `NewEnabled`/`NewEnabledCore`/`NewInputCore`, and
   `TensorAdjacency` state when a rule applies; `ProgressInvariant` keeps
   unallocated waiting cells undefined.
5. **Histories and the reservation ledger.** `SequentialFigure7History` and
   `TagHistory` give the proof-relevant execution history and its canonical
   tag augmentation (`CanonicalTagHistory`); `ReservationLedger`,
   `TouchOrigin`, `RawMarkHistory`, `RawMarkReservationAnchor`,
   `RawMarkCausalOrder`, and `QueueHistory` make every initialization or
   `new` an unforgeable event ordered by raw age and recover the event behind
   every touch, raw mark, and queued endpoint.
6. **Source-left geometry.** `SequentialFreshSourceLeftRun`,
   `SequentialFreshSourceBlocker`, `SequentialFigure7BlockerHistory`,
   `NewRegion`, `FreshCapacity`, `TerminalPartnerGeometry`,
   `TouchCompleteness`, `SameRepresentativeEventTouch`, and the
   `SequentialComponent*Geometry` modules describe the structural region a
   `NEXTAXIOM` search explores and prove it complete for its region.
7. **Region and commitment invariants.** The
   `OlderEventFutureWorkTouch*`, `OlderRawMarkedRegion*`,
   `OlderMarkedTensorPredecessor*`, `CrossRepresentative*`, and
   `ActiveRegion*` families are state or history invariants proved by
   initialization plus preservation through each creating rule; the
   `Commitment*` modules make the `sigma` spine exact, recovering the
   historical `new` event and a canonical reference path behind every adjacent
   retained boundary pair.
8. **Active-top debt and the tail law.** `ActiveTopResidual` names the
   drained boundary and proves
   `ReachableByImplementedDispatcher.dispatch_or_activeTopDrained`;
   `ActiveTopMarkedNonconclusionDebt` and `ActiveTopDebtHistoryTail` reduce
   progress to `CanonicalTagHistory.ActiveTopDebtTailLaw`. The
   `ActiveTopDebtParent*`, `CommitmentIntervalPar*`,
   `MarkedTarget*`, and `WaitingReentryContinuation*` families classify the
   `nop`/`wait` re-entry target under exact ready-tail failure; they are
   conditional classifications and do not discharge the law.
   `Figure7/TailLaw` states C12, proves it for every correct initialization,
   shows it discharges the `nop`/`wait` obligations, and certifies that it is
   not state-only inductive.
9. **Termination.** `Figure7/Termination` bounds every executed history by
   `formulas.size` dispatcher calls (`dispatch_stops`).
10. **Finite audits.** `proofnet_ir_new_progress_audit` replays every
    post-initialization state of its certificate sets and classifies ready-head
    availability; its `--cross-representative-search` mode checks the
    region-separation invariants on decoded transitions.
    `proofnet_ir_tail_law_search` searches for tail-law violations over
    derivation-generated certificates (`--wait-focus` for wait-dense sets) and
