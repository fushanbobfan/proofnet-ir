...
termination, or totality result. The continuation and sibling maps are recorded
next; typed-Wait transport remains unproved.
...
remain unresolved. The sibling map is recorded next; typed-Wait transport
remains unproved.
`SequentialFigure7MarkedTargetWaitingMateExternalCommitmentReentryContinuationSiblingQueueStatus.lean`
adds one public target and one public adapter, both with exactly the standard-
three boundary: `propext`, `Classical.choice`, and `Quot.sound`. The adapter
consumes an existing kernel-checked sibling temporal target plus the scheduler
invariant, component lookup, occurrence witness, and no-tail premise. It copies
the raw, older-future, and older-marked exits and changes only the causal
branch's nested continuation outcome through `queueStatusOutcome`, preserving
every outer receipt and the cyclic-junction causal evidence. It adds no runtime
oracle, native computation, source fact, correctness, current-mate-outside,
connectedness, par-kind, stored-side, geometric, path-alignment, or boundary-
equality premise. It proves no queue origin, queue history, persistence,
reachability, elimination, payer, tail law, progress, completion, termination,
or totality result. The three noncausal exits remain queue-unclassified and
uneliminated; typed-Wait transport remains unproved.
