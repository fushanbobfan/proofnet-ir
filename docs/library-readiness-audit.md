...
3. `Graph.Acyclic` is now exposed as absence of an exact stored-edge
   ...
   occurrence-exact `SchedulerInvariant`, proof-relevant histories with an
   unforgeable reservation ledger, region and commitment invariants proved by
   initialization and preservation, a termination bound of
   `certificate.formulas.size` dispatcher calls (D4, closed), and the
...
- experimenting with the sequential Figures 7–8 layer: the proof-bearing
  bounded/tagged `NEXTAXIOM` search, the delayed raw-age stack, the canonical
  six-rule dispatcher with its typed step witnesses and preserved
  `SchedulerInvariant`, proof-relevant histories, and the finite audits, while
  treating search failure as inconclusive and every scheduler theorem as
  conditional on its stated invariant and history hypotheses; exhaustive
  enabledness, later-state totality, progress, and completeness are not
  available, so together these are not a complete scheduler API;
