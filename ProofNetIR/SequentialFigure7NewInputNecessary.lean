...
This module separates two declarative input views from the operational
`NewExecutableEnabled` used by the canonical dispatcher.
...
condition.  It is necessary for executable `new`, but not sufficient.  In
particular it does not inspect the `NEXTAXIOM` route or any tag beyond what is
...
theorem NewExecutableEnabled.inputNecessary
  ...
  exact new?_success_implies_inputNecessary
    invariant.toReservationInvariant equation
...
theorem PriorityEnabled.newInputNecessary
    ...
    NewInputNecessary certificate before := by
  cases enabled with
  | new _ _ executable => exact executable.inputNecessary
