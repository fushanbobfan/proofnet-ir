...
/-!
# Figure-7 waiting-mate commitment re-entry continuation exit

The endpoint-parametric temporal target can be normalized through its finite
marked-conclusion continuation. Raw endpoints either leave the active carrier
or return exactly to the selected/current-mate pair; future and marked-global
endpoints remain outside at a strictly older boundary or representative.

The result retains the exact ready-tail-failure premise. It does not eliminate
future or marked endpoints, recover a payer, derive the history-tail law, or
establish progress, completion, termination, or totality.
-/
...
re-entry target. -/
def ActiveCarrierExternalReentryMarkedOuterMateSeparatedContinuationExitTarget
...
theorem continuationExitTarget
    ...
    (occurrence :
      Certificate.ComponentOccurrenceWitness certificate component
        usedLinks owned)
    (target :
      ActiveCarrierExternalReentryMarkedOuterMateSeparatedTemporalTarget tagHistory
        input component owned endpoint current)
    (noTail :
      ¬ ∃ pending,
